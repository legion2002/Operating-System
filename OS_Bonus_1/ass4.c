...
int lastIndex = -1;
int queueSize = 0;
struct tanishkNode *q;
int queueMade = 0;
int readerVal = 0;
DEFINE_MUTEX(mutex);
DEFINE_MUTEX(db);

int enqueue (struct tanishkNode * value)
{
	printk("Reached Enqeueue! \n");
	if(queueMade == 0){
		q = (struct tanishkNode *)kmalloc(numElements * sizeof(struct tanishkNode), GFP_KERNEL);
		queueMade = 1;		
	}
	if (queueSize ==  0) {		
		frontIndex = 0;
...
int dequeue(struct tanishkNode * pointer){
	printk("Reached Dequeue! \n");
	if(queueSize == 0 ){
		printk("Queue is Empty!");
	}
	else{
		struct tanishkNode popped =  q[frontIndex];
		frontIndex++;
...
SYSCALL_DEFINE1(writer, char * ,value){
	printk("Reached writer successfully!\n");
	char kernelBuff[8];
	int check = __copy_from_user(kernelBuff, value, sizeof(char)*8);
	if(check != 0){
		printk("Some error in copying from user\n");
	}
	else{
		printk("Copied from User successfully\n");
		
	}
	struct tanishkNode t;
	t.value = kernelBuff;
	//Mutex locks here
	mutex_lock(&db);
	enqueue(&t);
...
SYSCALL_DEFINE1(reader, char *, value){
	printk("Reached reader successfully!\n");
	struct tanishkNode * t;
	mutex_lock(&mutex);
	readerVal++;
	if(readerVal == 1){
	...
	int check = __copy_to_user(value, returnVal, sizeof(char) *8);
	if(check != 0){
		printk("Some error in copying to user\n");
	}
	else{
		printk("Copied to User successfully\n");
	}
	return 1;
