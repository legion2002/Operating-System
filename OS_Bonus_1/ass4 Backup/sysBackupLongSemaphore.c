...
DEFINE_MUTEX(mutex);
DEFINE_MUTEX(db);
DEFINE_MUTEX(checkEmpty);
struct semaphore counter;

int enqueue (long value)
{
	printk("Reached Enqueue, Queue Size: %d, FrontIndex: %d, LastIndex: %d\n", queueSize, frontIndex, lastIndex);
	
	if(queueMade == 0){
		printk("Making Queue\n");
		sema_init(&counter,0);
		q = (long *)kmalloc(numElements * sizeof(long), GFP_KERNEL);
		queueMade = 1;
			
	}
	
	if (lastIndex  == frontIndex  && lastIndex != -1){
		printk("Queue is full! Queue Size: %d, FrontIndex: %d, LastIndex: %d\n", queueSize, frontIndex, lastIndex);
	}
	else{
		if(lastIndex == -1){
			lastIndex = 0;
		...
		q[lastIndex] = value;
		long test = q[lastIndex];
		printk("Added Value to Queue: %ld \n", test);
		if(frontIndex == -1){
			frontIndex++;
		}
		lastIndex++;
...
int dequeue(void){
	printk("Reached Dequeue, Queue Size: %d, FrontIndex: %d, LastIndex: %d\n", queueSize, frontIndex, lastIndex);
	if(queueSize == 0 ){
		printk("Queue is Empty! Queue Size: %d, FrontIndex: %d, LastIndex: %d\n", queueSize, frontIndex, lastIndex);
		frontIndex = -1;
		lastIndex = -1;
		popped = 0;
	}
		...
		popped = q[frontIndex];
		frontIndex++;
		if(frontIndex == numElements){
			frontIndex -= numElements;
		}
		queueSize--;
		
		printk("Received Value from Queue: %ld \n", popped);
...
SYSCALL_DEFINE1(writer, long ,value){
	printk("Writer: QueueSize: %d, QueueMade: %d\n", queueSize, queueMade);
	printk("Enqueue: %ld, to QueueSize: %d, QueueMade: %d\n", value, queueSize, queueMade);
	//Mutex locks here
...
SYSCALL_DEFINE1(reader, long, value){
	down(&counter);
	printk("Reader: QueueSize: %d, QueueMade: %d\n", queueSize, queueMade);
	mutex_lock(&mutex);
	readerVal++;
	if(readerVal == 1){
		mutex_lock(&db);
	...
	long returnVal;
	returnVal = popped;
	printk("Got this val: %ld, to QueueSize: %d, QueueMade: %d\n", returnVal, queueSize, queueMade);
